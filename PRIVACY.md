...
后台同步只上传收藏夹结构、标题、URL 和收藏时间。WXT 内容脚本匹配所有 `http://*/*` 和 `https://*/*` 页面，因此会被浏览器注入这些页面；注入后平时只注册消息监听并保持空闲。只有用户在 popup 中明确点击“收藏当前网页并保存正文”，popup 向当前标签页发送 `bookmark-recall:capture` 消息后，内容脚本才克隆并清理 DOM，再提取可读纯文本、标题层级、描述和语言。表单控件、脚本和完整 DOM 不会作为采集载荷保存。
