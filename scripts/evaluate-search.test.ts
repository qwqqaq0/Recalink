...
describe("search evaluation metrics", () => {
  it("calculates Hit@1, Hit@5 and Hit@10 from ranked URLs", () => {
    const metrics = calculateHitMetrics([
      ...
      {
        expectedUrl: "https://b.test",
        rankedUrls: ["https://x.test", "https://b.test"]
      },
      {
        expectedUrl: "https://c.test",
        rankedUrls: Array.from({ length: 6 }, (_, index) =>
          index === 5 ? "https://c.test" : `https://${index}.test`
        )
      },
