...
export const capturePayloadSchema = z.object({
  url: z.url(), title: z.string().trim().max(500),
  description: z.string().max(2_000).default(""), plainText: z.string().max(500_000),
...
export interface EdgeSyncNode {
  id: string; parentId?: string | undefined; title: string; url?: string | undefined;
  dateAdded?: number | undefined; children?: EdgeSyncNode[] | undefined;
}

const edgeNodeFields = {
  id: z.string().min(1).max(512), parentId: z.string().max(512).optional(),
  title: z.string().max(500), url: z.url().optional(), dateAdded: z.number().nonnegative().optional()
};
export const edgeSyncNodeSchema: z.ZodType<EdgeSyncNode> = z.lazy(() =>
  z.object({ ...edgeNodeFields, children: z.array(edgeSyncNodeSchema).optional() })
);
export const edgeSyncSchema = z.object({ nodes: z.array(edgeSyncNodeSchema).max(100_000) });
...
export const searchQuerySchema = z.object({
  ...
  limit: z.coerce.number().int().min(1).max(100).default(20), ai: queryBoolean.default(false),
  folderId: z.string().uuid().optional(), tagIds: z.array(z.string().uuid()).max(50).optional(),
  domain: z.string().trim().max(255).optional(),
  captureStatus: z.enum(["pending", "processing", "ready", "metadata_only", "failed"]).optional(),
  createdFrom: z.iso.datetime().optional(), createdTo: z.iso.datetime().optional()
});
...
export const aiRerankSchema = z.object({ results: z.array(z.object({
  bookmarkId: z.string().min(1), reason: z.string().trim().max(300)
})).max(20) });
...
export const bookmarkPatchSchema = z.object({
  ...
  note: z.string().max(10_000).optional(), tagIds: z.array(z.string().uuid()).max(50).optional()
});
export const createBookmarkSchema = z.object({
  url: z.url(), title: z.string().trim().max(500).default(""), note: z.string().max(10_000).default("")
});
