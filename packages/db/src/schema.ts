...
export const bookmarkSources = pgTable(
  "bookmark_sources",
  {
    id: uuid("id").primaryKey().defaultRandom(),
...
export const aiTagSuggestions = pgTable(
  "ai_tag_suggestions",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    bookmarkId: uuid("bookmark_id")
      .notNull()
      .references(() => bookmarks.id, { onDelete: "cascade" }),
    existingTagId: uuid("existing_tag_id").references(() => tags.id, {
      onDelete: "cascade"
    }),
...
export const bookmarkRelations = relations(bookmarks, ({ many, one }) => ({
  ...
  content: one(pageContents, {
    fields: [bookmarks.id],
    references: [pageContents.bookmarkId]
  })
}));

export const sourceRelations = relations(bookmarkSources, ({ one }) => ({
  bookmark: one(bookmarks, {
    fields: [bookmarkSources.bookmarkId],
    references: [bookmarks.id]
  }),
  folder: one(folders, {
    fields: [bookmarkSources.folderId],
