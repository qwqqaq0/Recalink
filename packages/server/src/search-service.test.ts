...
import { SearchService, type SearchBackend, type SearchHit } from "./search-service.js";

const hit = (id: string): SearchHit => ({
  id, title: `title-${id}`, url: `https://example.com/${id}`,
  domain: "example.com", snippet: `snippet-${id}`,
  matchedFields: ["plainText"], matchedTerms: [id]
});

describe("SearchService", () => {
  it("expands, fuses, and safely reranks candidates", async () => {
    const backend: SearchBackend = {
      search: async (query) => query === "original" ? [hit("a"), hit("b")] : [hit("b"), hit("c")]
    };
    ...
    const result = await service.search({ q: "original", ai: true, page: 1, limit: 20 });
  ...
  it("falls back to basic search when AI fails", async () => {
    ...
    const service = new SearchService(backend, {
      expand: async () => { throw new Error("offline"); }, rerank: async () => []
    });
    const result = await service.search({ q: "original", ai: true, page: 1, limit: 20 });
