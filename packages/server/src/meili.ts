...
export class MeiliBookmarkIndex implements SearchBackend {
  ...
  async put(document: BookmarkSearchDocument): Promise<void> {
    const task = await this.client
      .index<BookmarkSearchDocument>(this.indexName)
  ...
  async search(
    query: string,
    options: BackendSearchOptions = { page: 1, limit: 20 }
  ): Promise<SearchHit[]> {
    const filters: string[] = [];
    if (options.folderId)
      filters.push(`folderIds = ${JSON.stringify(options.folderId)}`);
    if (options.tagIds?.length)
      filters.push(
        `tagIds IN [${options.tagIds.map((id) => JSON.stringify(id)).join(",")}]`
      );
    if (options.domain)
      filters.push(`domain = ${JSON.stringify(options.domain)}`);
    if (options.captureStatus)
      filters.push(`captureStatus = ${JSON.stringify(options.captureStatus)}`);
    if (options.createdFrom)
      filters.push(
        `createdAtEpoch >= ${new Date(options.createdFrom).getTime()}`
      );
    if (options.createdTo)
      filters.push(
        `createdAtEpoch <= ${new Date(options.createdTo).getTime()}`
      );
