      ...
      : [
          ...(node.id !== "0"
            ? [
                {
                  id: node.id,
                  label: `${"　".repeat(Math.max(0, depth - 1))}${node.title || "根目录"}`
                }
              ]
            : []),
...
function Popup() {
  ...
  async function chooseFolder(value: string) {
    setFolderId(value);
    await browser.storage.local.set({ folderId: value });
  }
  ...
  async function search() {
    ...
    setMessage("");
    try {
      const payload = await apiRequest<{ items: SearchHit[] }>(
        `/search?q=${encodeURIComponent(query)}&ai=false&limit=5`,
        undefined,
        false
      );
