...
describe("bookmark title helpers", () => {
  it("prefers a trimmed non-empty tab title", () => {
    expect(chooseDefaultBookmarkTitle("  Useful article  ", "https://example.com")).toBe(
      "Useful article"
    );
  });

  it("falls back to the URL for undefined or whitespace-only tab titles", () => {
    expect(chooseDefaultBookmarkTitle(undefined, " https://example.com/one ")).toBe(
  ...
  it("trims and caps bookmark titles at the maximum length", () => {
    expect(normalizeBookmarkTitle(`  ${"a".repeat(MAX_BOOKMARK_TITLE_LENGTH + 1)}  `)).toBe(
      "a".repeat(MAX_BOOKMARK_TITLE_LENGTH)
    );
  });
